...
struct ChampsimTageScl {
  ...
  ChampsimTageScl(std::size_t max_inflight_branches)
      : impl(max_inflight_branches), id(0), state(NONE),
        tmp_id(0), tmp_ip(0), tmp_prediction(false), tmp_has_pending(false),
        has_pending(false), pending_id(0), pending_ip(0), pending_prediction(false), is_first_prediction(true) {}
  ...
  std::uint64_t   tmp_ip;
  bool            tmp_prediction;
  bool            tmp_has_pending;


  bool          has_pending;
  std::uint32_t pending_id;
  std::uint64_t pending_ip;
  bool          pending_prediction;
  bool          is_first_prediction;
};
...
std::uint8_t O3_CPU::predict_branch(std::uint64_t ip) {
  ...
  if (predictor.state == ChampsimTageScl::PREDICTED) {
    ...
    type.is_conditional = false;
    type.is_indirect    = false;
    predictor.impl.commit_state_at_retire(
        predictor.id, predictor.last_ip, type, 0, 0,  predictor.is_first_prediction);
  }

  // Allocate slot and compute prediction for ip_N,
  // this will be served next cycle as the two-ahead prediction
  predictor.id = predictor.impl.get_new_branch_id();
  bool next_prediction = predictor.impl.get_prediction(predictor.id, ip);

  // Serve what was computed last cycle (for ip_{N-1}), using ip_{N-2}'s context
  bool prediction;
  if (predictor.has_pending) {
    prediction = predictor.pending_prediction;
    predictor.is_first_prediction = false;
  } else {
    // No pending prediction yet, fall back to current
    prediction           = next_prediction;
    predictor.pending_id  = predictor.id;
    predictor.pending_ip  = ip;
    predictor.is_first_prediction = true;
    predictor.has_pending = true;
  }

  // Store current branch as pending for next cycle
  predictor.tmp_id          = predictor.id;
  predictor.tmp_ip          = ip;
  predictor.tmp_prediction  = next_prediction;
  predictor.tmp_has_pending = true;

  predictor.last_ip = ip;
  predictor.state   = ChampsimTageScl::PREDICTED;
  return static_cast<std::uint8_t>(prediction);
}
...
void O3_CPU::last_branch_result(std::uint64_t ip, std::uint64_t target,
  ...
  type.is_indirect =
      branch_type == BRANCH_INDIRECT    || branch_type == BRANCH_INDIRECT_CALL ||
      branch_type == BRANCH_RETURN      || branch_type == BRANCH_OTHER;

 
  predictor.impl.update_speculative_state(
      predictor.pending_id, predictor.pending_ip, type, taken, target);

  if (type.is_conditional) {
    predictor.impl.commit_state(
        predictor.pending_id, predictor.pending_ip, type, taken);
  }

  predictor.impl.commit_state_at_retire(
      predictor.pending_id, predictor.pending_ip, type, taken, target, predictor.is_first_prediction);

  predictor.state = ChampsimTageScl::NONE;

  // Store current branch as pending for next cycle
  predictor.pending_id         = predictor.tmp_id;
  predictor.pending_ip         = predictor.tmp_ip;
  predictor.pending_prediction = predictor.tmp_prediction;
  predictor.has_pending        = predictor.tmp_has_pending;
}