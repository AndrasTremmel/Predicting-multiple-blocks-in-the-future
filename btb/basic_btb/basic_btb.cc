...
//   std::map<O3_CPU*, std::bitset<champsim::lg2(BTB_INDIRECT_SIZE)>> CONDITIONAL_HISTORY;
//   std::map<O3_CPU*, std::deque<uint64_t>> RAS;
//   std::map<O3_CPU*, std::unordered_map<uint64_t, uint8_t>> CALL_SIZE;
//   std::map<O3_CPU*, std::deque<pending_btb_pred_t>> PRED_QUEUE;
//   std::map<O3_CPU*, btb_stats_t> STATS;
...
//       } else {
//         ras_empty = false;
//         uint64_t target = g_ctx.RAS[this].back();
//         auto it = g_ctx.CALL_SIZE[this].find(target);
//         auto size = (it != g_ctx.CALL_SIZE[this].end()) ? it->second : 4;  // 4 = sane default
//         predicted_target = target + size;
//         always_taken = true;
//       }
//     }
...
//     auto call_ip = g_ctx.RAS[this].back();
//     g_ctx.RAS[this].pop_back();
//     auto sz = (call_ip > branch_target) ? call_ip - branch_target : branch_target - call_ip;
//     if (sz <= 10) g_ctx.CALL_SIZE[this][call_ip] = static_cast<uint8_t>(sz);
//   }
