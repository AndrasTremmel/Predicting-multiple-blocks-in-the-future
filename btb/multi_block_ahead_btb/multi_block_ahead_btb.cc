...
struct btb_entry_t {
  ...
  auto tag() const {
    return (ip_tag >> 2) | (static_cast<uint64_t>(transition) << 48);
  }
};
...
std::pair<uint64_t, uint8_t> O3_CPU::btb_prediction(uint64_t ip)
{
  ...
  bool always_taken = (btb_entry->type != ::branch_info::CONDITIONAL);
  return {btb_entry->target, always_taken};
}
                        ...
                        uint8_t taken, uint8_t branch_type)
{
  ...
  if (branch_target != 0) {
    ::BTB.at(this).fill(opt_entry.value_or(new_entry));
  }
}