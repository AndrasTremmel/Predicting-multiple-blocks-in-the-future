...
//   uint64_t sas_pops = 0;
//   uint64_t sas_empty_on_pop = 0;
//   uint64_t sas_context_used = 0;
//   uint64_t sas_context_missed = 0;
// };
...
// uint64_t get_context_ip(O3_CPU* cpu, uint64_t prev_ip, mbtb_transition trans)
// {
//   // SAS-based context (paper's missing case)
//   if (trans == mbtb_transition::R && !g_ctx.SAS[cpu].empty()) {
//     return g_ctx.SAS[cpu].back();
//   }
...
//   auto context_ip = get_context_ip(this, prev_ip, trans);

//   auto entry = g_ctx.MBTB.at(this).check_hit({context_ip, 0, branch_info::ALWAYS_TAKEN, trans});
...
//   g_ctx.PRED_QUEUE[this].push_back({
//       ip,
//       prev_ip,
//       context_ip,   // NEW
//       trans,
...
//     if (pred.context_ip != pred.prev_ip)
//       stats.sas_context_used++;
//     else
//       stats.sas_context_missed++;
...
//   // // Use the exact prediction-time context
//   // uint64_t update_context_ip =
//   //     found ? pred.context_ip : g_ctx.LAST_BRANCH_IP[this];

//   // mbtb_transition update_transition =
//   //     found ? pred.trans : g_ctx.LAST_TRANSITION[this];

//   // auto opt_entry = g_ctx.MBTB.at(this).check_hit(
//   //     {update_context_ip, branch_target, type, update_transition});

//   // Reconstruct the SAME context generation logic used at prediction time
//   uint64_t update_context_ip;
//   mbtb_transition update_transition;

//   if (found) {
//       // Exact prediction-time context
//       update_context_ip = pred.context_ip;
//       update_transition = pred.trans;
//   } else {
//       // Fallback path must still use SAS-aware context logic
//       auto prev_ip = g_ctx.LAST_BRANCH_IP[this];
//       auto prev_trans = g_ctx.LAST_TRANSITION[this];

//       update_context_ip =
//           get_context_ip(this, prev_ip, prev_trans);

//       update_transition = prev_trans;
//   }
